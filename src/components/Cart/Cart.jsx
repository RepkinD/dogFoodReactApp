...
function Cart() {
  ...
  const {
    ...
    queryFn: () =>
      DogFoodApiConst.getProductsByIds(
  ...
  const isAllCardPicked = () =>
    cart.filter((item) => item.isPicked === true).length === cart.length;
  ...
  const getCartProductById = (idItem) =>
    cartProducts.find((product) => product._id === idItem);
  const getCartStateProductById = (idItem) =>
    cart.find((product) => product.id === idItem);
  const pickAllProductsHandler = () => {
    if (!isAllCardPicked()) dispatch(pickAllProducts());
    else dispatch(notPickAllProducts());
  };
  const calculateSum = () =>
    findAllPickedProducts().reduce((sum, product) => {
      const updatedSum =
        sum + product.count * getCartProductById(product.id).price;
      return updatedSum;
    }, 0);
  ...
  const calculateSumWithDiscount = () =>
    findAllPickedProducts().reduce((sum, product) => {
      const updatedSum =
        sum +
        product.count *
        ...
        {cartProducts[0] && (
          ...
          >
            ...
            >
              ...
              <p>Сумма: {calculateSum() || 0} ₽</p>
              <p>Скидка: {calculateDiscount() || 0} ₽</p>
              <h5>К оплате: {calculateSumWithDiscount() || 0} ₽</h5>
